...
public class AnswerQuestionPMIScorer extends JCasAnnotator_ImplBase {
  ...
  public void process(JCas aJCas) throws AnalysisEngineProcessException {
    ...
    for (int i = 0; i < qaSet.size(); i++) {
      ...
      for (int c = 0; c < topK; c++) {
        ...
        ArrayList<NounPhrase> questionNouns = Utils
            .fromFSListToCollection(question.getNounList(), NounPhrase.class);
        ArrayList<NER> questionNers = Utils.fromFSListToCollection(
        ...
        System.out.println("length of sentencs: " + questionNouns.size());
        *
        *
        */
        ...
        for (int j = 0; j < choiceList.size(); j++) {
          ...
          for (int k = 0; k < questionNouns.size(); k++) {
            try {
              score1 += scoreCoOccurInSameDoc(questionNouns
          ...
          for (int k = 0; k < questionNers.size(); k++) {
            ...
            try {
              score1 += scoreCoOccurInSameDoc(questionNers.get(k)
